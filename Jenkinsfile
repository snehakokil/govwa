      ...
      parallel
      {
        ...
        stage('5. Running ZAP')
        {
          ...
          //  sh 'zap-baseline.py -t http://localhost:8082 -r  baseline-scan-report.html '
              sh 'zap-cli report -help '
              sh 'zap-cli -p 8090 -v quick-scan -sc -l Informational -o \'-config api.disablekey=true\' http://localhost:8082 && zap-cli report -o test.json -f json'
              echo 'zap complete'
              }
          catch (err)
          {
